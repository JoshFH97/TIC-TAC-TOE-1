    ...
    cuadrados.forEach((cuadrado, i) => {
        if (!bandera) {//para hacer pausa guado alguien gane
            cuadrado.addEventListener('click', () => {
                if (!bandera && cuadrado.innerHTML == "") {//si nadie ha ganado y el cuadro esta vacio se puede clickear si no no.
                    cuadrado.innerHTML = turn === 'player1' ? x : o;
                    turn = turn === 'player1' ? 'computer' : 'player1';

                    revisarGanador()

                    if (turn === 'computer' && !bandera) {//Si es el turno de la computadora y nadie ha ganado, hacer un movimiento automático
...
//esto es para que cuando yo le haga click en la img del player me la guarde en el local storage
//y asi poder extraerla para que la use en el gato en vez de O/X.
